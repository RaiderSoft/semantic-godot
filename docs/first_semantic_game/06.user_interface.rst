...
* Write, send, and receive messages while in a conversation with another ``ChatEntity``.
...
Set the following property so that the ``HSplitContainer`` takes up the whole screen:
...
``SubViewportContainer`` so that the level will expand to fill the left 80% of the screen:
